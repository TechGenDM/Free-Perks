...
export function SceneBuild({ sections }: SceneBuildProps) {
  ...
  return (
    <section className="relative py-28 md:py-36 px-4 md:px-6 bg-[#090d16] overflow-hidden border-t border-white/4">
      {/* Ambience */}
      <div className="aurora-orb-cyan top-[20%] -right-30" />
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        >
          ...
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-extrabold tracking-[-0.03em] text-white mb-4">
            Everything you need.{' '}
            <span className="bg-linear-to-r from-cyan-300 via-indigo-300 to-purple-300 bg-clip-text text-transparent">
              Nothing you owe.
            </span>
          </h2>
              ...
              <button
                ...
                className={`relative px-5 py-2.5 rounded-2xl text-xs sm:text-sm font-semibold transition-all duration-200 cursor-pointer shrink-0 border ${
                  isActive
                    ...
                    : 'text-slate-400 border-white/8 bg-white/3 hover:text-slate-200 hover:border-white/20'
                }`}
              >
        ...
        >
          {currentSection.tools.map((tool) => (
            <Link
              ...
              className="group block p-5 rounded-2xl glass-card border border-white/8 hover:border-indigo-500/50 transition-all duration-300"
            >
